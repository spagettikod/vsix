...
📣
[Remote attach debugging](https://github.com/golang/vscode-go/blob/HEAD/docs/debugging.md#connecting-to-headless-delve-with-target-specified-at-server-start-up) is now available via Delve's native DAP implementation with Delve v1.7.3 or newer.
We plan to enable this as the default in early 2022 to enhance remote debugging with the same
[debugging features](https://github.com/golang/vscode-go/blob/HEAD/docs/debugging.md) that are already in use for local debugging.
